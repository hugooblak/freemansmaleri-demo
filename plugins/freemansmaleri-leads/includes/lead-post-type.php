...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'freemansmaleri-leads' ),
					'singular_name' => __( 'Lead', 'freemansmaleri-leads' ),
					'edit_item'     => __( 'Förfrågan', 'freemansmaleri-leads' ),
					'search_items'  => __( 'Search leads', 'freemansmaleri-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'freemansmaleri-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'freemansmaleri-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'freemansmaleri-leads' ),
			'npl_phone'    => __( 'Phone', 'freemansmaleri-leads' ),
			'npl_service'  => __( 'Needs', 'freemansmaleri-leads' ),
			'npl_timeline' => __( 'When', 'freemansmaleri-leads' ),
			'npl_zip'      => __( 'ZIP', 'freemansmaleri-leads' ),
			'npl_source'   => __( 'Source', 'freemansmaleri-leads' ),
			'npl_status'   => __( 'Status', 'freemansmaleri-leads' ),
			'date'         => __( 'Received', 'freemansmaleri-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'freemansmaleri-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'freemansmaleri-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'freemansmaleri-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'freemansmaleri-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'freemansmaleri-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'freemansmaleri-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'freemansmaleri-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'freemansmaleri-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'freemansmaleri-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'freemansmaleri-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'freemansmaleri-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'freemansmaleri-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
