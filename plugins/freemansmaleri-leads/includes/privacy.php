...
add_filter(
	...
	function ( $exporters ) {
		$exporters['freemansmaleri-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'freemansmaleri-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'freemansmaleri-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'freemansmaleri-leads',
						'group_label' => __( 'Quote requests', 'freemansmaleri-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['freemansmaleri-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'freemansmaleri-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
